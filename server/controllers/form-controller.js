...
import { DynamicWebsite } from "../mongodb/schema/Dynamic.js";
...
export const DynamicForm = async (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return next(
      new HttpError("Invalid inputs passed, please check your data.", 422)
  ...
  try {
    existingSubmit = await DynamicWebsite.findOne({ register_no });
  } catch (err) {
    const error = new HttpError(
      "Something went wrong, please try again later.",
      500
  ...
  if (existingSubmit) {
    ...
    try {
      newForm = new DynamicWebsite({ name, register_no, githubLink, type });
    } catch (err) {
      const error = new HttpError(
        "Something went wrong, please try again later.",
        500
